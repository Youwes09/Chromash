...
#[derive(Debug, Copy, Clone, Serialize, Deserialize)]
pub enum SchemeType { Neutral, Rainbow, Monochrome, Analogous }
...
impl SchemeType {
    fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::Analogous => "scheme-analogous",
        }
    }
    fn from_str(s: &str) -> Option<Self> {
        match s.to_lowercase().as_str() {
    ...
    fn from_chroma(r: u8, g: u8, b: u8) -> Self {
        ...
        if chroma < 50 { Self::Neutral } else { Self::Rainbow }
    }
}
...
impl Config {
    ...
    fn last_wallpaper() -> PathBuf {
        Self::home().join(".config/hyprpaper/SelectedWallpaper.png")
    }
...
impl ChromashApi {
    pub fn new() -> Result<Self> {
        let dirs = [Config::config_dir(), Config::presets_dir(), Config::wallpaper_dir()];
        for dir in &dirs {
    ...
    pub fn apply_color(&mut self, color: &str, options: ThemeOptions) -> Result<bool> {
        ...
        let scheme = options.scheme.unwrap_or(SchemeType::Rainbow);
    ...
    fn select_wallpaper(&self, path: Option<&str>) -> Result<PathBuf> {
        ...
        let last = Config::last_wallpaper();
        if last.is_file() { return Ok(last); }
    ...
    fn set_wallpaper(&self, path: &Path) -> Result<()> {
        let dest = Config::last_wallpaper();
        ...
        self.run_command("hyprctl", &["hyprpaper", "preload", &path_str])?;
